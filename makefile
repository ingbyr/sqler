...
build-linux: generate
	CGO_ENABLE=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -extldflags "-static" -X 'main.Version=`git log --pretty=format:'%H' -1`' -X 'main.BuildTime=`date '+%Y-%m-%d %H:%M:%S'`' -X 'main.BuildBy=`go version`'" -o bin/sqler .
